...
class KNN:
    ...
    def get_k_neighbors(self, point):
        ...
        distances = []
        neigh = []
        for idx, train_pt in enumerate(self.features):
            dist = self.distance_function(point, train_pt)
            distances.append((dist,idx,self.labels[idx]))
        distances.sort(key = lambda x: (x[0], x[1]))
        for _, _, label in distances[:self.k]:
            neigh.append(label)
        return neigh
    ...
    def predict(self, features):
        ...
        preds = []
        for pt in features:
            neigh = self.get_k_neighbors(pt)
            label = Counter(neigh).most_common(1)[0][0]
            preds.append(label)
        return preds
