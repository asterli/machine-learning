...
class HMM:
    ...
    def forward(self, Osequence):
        ...
        for t in range(1, L):
            for j in range(S):
                alpha[j,t] = self.B[j,O[t]]*np.sum(alpha[:,t-1]*self.A[:,j])
        return alpha
    ...
    def likelihood_prob(self, Osequence):
        ...
        for t in range(L-1):
            for i in range(S):
                for j in range(S):
                    xi[i,j,t]=(alpha[i,t]*self.A[i,j]*self.B[j,O[t+1]]*beta[j,t+1])/prob
    ...
    def viterbi(self, Osequence):
        ...
        for t in range(1,L):
            for j in range(S):
                ...
                psi[j,t] = np.argmax(vals)
                delta[j,t]=np.max(vals)*self.B[j,O[t]]
        states = np.zeros(L,dtype=int)
        states[-1] = np.argmax(delta[:,-1])
        for t in range(L-2,-1,-1):
            states[t] = psi[states[t+1],t+1]
        for s in states:
            path.append(self.find_key(self.state_dict,s))
        return path
